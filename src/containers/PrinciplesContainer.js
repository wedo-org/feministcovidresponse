...
{/******************************* SECOND PRINCIPLE ******************************* */}
                <div className='accordion-item' onClick={()=>toggleClass()}>
                    <section className='blob-principle'>
                        <img src={require('../assets/images/orange-blob.png')} className='orange-blob' alt=''/>
                        <a className={`${isActive ? 'active' : null}`}>{t('COVID-19 responses must ensure the health and safety of all, including ensuring sexual and reproductive health and rights')}</a>
                    </section>
                    <div className={`content ${isActive ? 'active' : null}`}>
                        <p className='principle-description'>{t('There can be no effective response to a public health crisis that does not center gender equality and vice versa. We are witnessing attacks on the human rights of women, girls, and gender non-conforming people by further restricting abortion, access to comprehensive sexuality education, and gender-affirming treatment. Restrictions on movement and/or social isolation measures without adequate social safety nets and support for care services have increased the burden of care work on women, and have resulted in surges in domestic, intimate partner, and other forms of gender-based violence across the globe. Suggested actions include')}:</p>
                            <ul>
...
{/******************************* THIRD PRINCIPLE ******************************* */}
                <div className='accordion-item' onClick={()=>toggleClass()}>
                    <section className='blob-principle'>
                        <img src={require('../assets/images/orange-blob.png')} className='orange-blob' alt=''/>
                        <a className={`${isActive ? 'active' : null}`}>{t('COVID-19 responses must promote a comprehensive paradigm shift, relying on adequate and equitable financing')}</a>
                    </section>
                    <div className={`content ${isActive ? 'active' : null}`}>
                        ...
                        <p className='principle-description'>{t('During the COVID-19 pandemic, it is more important than ever that developed countries fulfill their existing obligations under multiple international UN processes (such as Financing for Development, UNFCCC and the Sustainable Development Goals) to support developing countries financially for urgently needed action. There should be a clear commitment by developed countries to urgently increase financial provision in the form of grants. Actions include')}:</p>
                            <ul>
                                ...
                                <li className='principle-item'>{t('Prohibit corporate lobbying and undue influence on government responses to COVID-19 and halt price gouging. Ensure that economic stimulus measures are focused on supporting the well-being of all people, especially the most marginalized and at risk, rather than on large corporate bailouts. ')}</li>
...
{/******************************* FOURTH PRINCIPLE ******************************* */}
                <div className='accordion-item' onClick={()=>toggleClass()}>
                    <section className='blob-principle'>
                        <img src={require('../assets/images/orange-blob.png')} className='orange-blob' alt=''/>
                        <a className={`${isActive ? 'active' : null}`}>{t('COVID-19 responses must be based on and strengthen democratic values')}</a>
                    </section>
                    <div className={`content ${isActive ? 'active' : null}`}>
...
{/******************************* FIFTH PRINCIPLE ******************************* */}
                <div className='accordion-item' onClick={()=>toggleClass()}>
                    <section className='blob-principle'>
                        <img src={require('../assets/images/orange-blob.png')} className='orange-blob' alt=''/>
                        <a className={`${isActive ? 'active' : null}`}>{t('COVID-19 responses must promote a just and equitable transition for people and planet')}</a>
                    </section>
                    <div className={`content ${isActive ? 'active' : null}`}>
                        <p className='principle-description'>{t('COVID-19 responses must be a downpayment on a just and equitable transition towards an equal and healthy planet. The post-COVID future should not be a return to a carbon-intensive economic system based on patriarchy and neo-liberal capitalism. A just and equitable transition involves communities in decarbonization and the transformational shift from the privatization and commodification of resources toward regenerative, sustainable, cooperative, and collective models. A just and equitable transition must be focused beyond the obvious sectors of oil and mining, to include affected workers across the global chains and those whose livelihoods have already been disrupted by climate change, which often includes women working in fishing, agriculture and care sectors. It is these same communities that are likely to lose income and work due to COVID-19, or that are required to continue working, risking health and well-being. Actions include')}:</p>
                            <ul>
